...
import logging
import os
import sites   # needed for relative imports.
sites = sites   # defeat lint

# TODO PEP8, refactor, and tidy - This may be functional but is way too ugly!

SITE_ID = settings.__dict__['_wrapped'].__class__.SITE_ID = make_tls_property()
TEMPLATE_DIRS = settings.__dict__['_wrapped'].__class__.TEMPLATE_DIRS = \
  make_tls_property(settings.TEMPLATE_DIRS)
    ...
    def process_request(self, request):
        ...
        if self.site:
            ...
            if self.site.folder_name:
                ...
                try:
                    settings_pkg = '%s.settings' % folder_name
                    __import__("sites.%s" % settings_pkg)
                    self.logger.debug('using sites.%s for settings',
                        settings_pkg)
                    self.override_settings(settings, folder_name)
                except ImportError:
                    # settings don't exist... skip it
                    self.logger.debug(
                        'cannot find sites.%s.settings... skipping',
                        folder_name)
                    pass
